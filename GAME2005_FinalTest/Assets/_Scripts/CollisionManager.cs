...
public class CollisionManager : MonoBehaviour
{
    ...
    public static void CheckSphereAABB(BulletBehaviour s, CubeBehaviour b)
    {
        ...
        if ((distance < s.radius) && (!s.isColliding))
        {
            ...
            if (b.name != "Ground")
            {
                if ((s.collisionNormal == Vector3.right))
                {
                    s.direction = new Vector3(-s.direction.x, s.direction.y, s.direction.z);
                    b.gameObject.GetComponent<RigidBody3D>().addForcerightBullet();
                }

                if ((s.collisionNormal == Vector3.forward))
                {
                    s.direction = new Vector3(s.direction.x, s.direction.y, -s.direction.z);
                    b.gameObject.GetComponent<RigidBody3D>().addForceforwardBullet();
                }


                if ((s.collisionNormal == Vector3.back))
                {
                    s.direction = new Vector3(s.direction.x, s.direction.y, -s.direction.z);
                    b.gameObject.GetComponent<RigidBody3D>().addForcebackwardBullet();
                }

                if ((s.collisionNormal == Vector3.left))
                {
                    s.direction = new Vector3(-s.direction.x, s.direction.y, s.direction.z);
                    b.gameObject.GetComponent<RigidBody3D>().addForceleftBullet();
                }
            }

                if ((s.collisionNormal == Vector3.up) || (s.collisionNormal == Vector3.down)) {
                    s.direction = new Vector3(s.direction.x, -s.direction.y, s.direction.z);
                  }
    ...
    public static void CheckAABBs(CubeBehaviour a, CubeBehaviour b)
    {
            ...
            (a.min.z <= b.max.z && a.max.z >= b.min.z))
        {
            ...
            if (!a.contacts.Contains(contactB))
            {
                ...
                {
                    a.gameObject.GetComponent<RigidBody3D>().addForceleftBody();

                }

                if (contactB.face == Vector3.left)
                {
                    a.gameObject.GetComponent<RigidBody3D>().addForcerightBody();
                ...
                {
                    a.gameObject.GetComponent<RigidBody3D>().addForcebackwardBody();

                }

                if (contactB.face == Vector3.back)
                {
                    a.gameObject.GetComponent<RigidBody3D>().addForceforwardBody();
