        ...
        if ((distance < s.radius) && (!s.isColliding))
        {
            ...
            if (b.tag != "Ground")
            {
                ...
                if ((s.collisionNormal == Vector3.right))
                {
                    ...
                    b.gameObject.GetComponent<RigidBody3D>().addForceright();
                }

                if ((s.collisionNormal == Vector3.forward))
                {
                    s.direction = new Vector3(s.direction.x, s.direction.y, -s.direction.z);
                    b.gameObject.GetComponent<RigidBody3D>().addForceforward();
                }
                ...
                {
                    s.direction = new Vector3(s.direction.x, s.direction.y, -s.direction.z);
                    b.gameObject.GetComponent<RigidBody3D>().addForcebackward();
                }

                if ((s.collisionNormal == Vector3.left))
                {
                    s.direction = new Vector3(-s.direction.x, s.direction.y, s.direction.z);
                    b.gameObject.GetComponent<RigidBody3D>().addForceleft();
                }
